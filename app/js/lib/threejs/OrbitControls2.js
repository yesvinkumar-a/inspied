THREE.OrbitControls2 = function ( object ) {
    ...
    phi += 0.25 * Math.PI / 180;
		theta += 0.25 * Math.PI / 180;
    ...
    object.position.x = scope.radius * Math.cos( phi ) * Math.cos( theta ) * scope.scaleX / 1;
		object.position.y = scope.radius * Math.sin( phi ) * scope.scaleY;
		object.position.z = scope.radius * Math.cos( phi ) * Math.sin( theta ) * scope.scaleZ / 1;
